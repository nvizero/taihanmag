class CatesController < ApplicationController
  ...
  def index
    ...
    @cates = Cate.where.not(:id => 1).order(sort_column + " " + sort_direction).page(params[:page]).limit(8)
    @level = 1
  end
  ...
  private
    ...
    def cate_params
      params.require(:cate).permit(:name , :flag , :level , :ref_cate_id , :name_en ,:content ,:content_en, :img)
    end
end
